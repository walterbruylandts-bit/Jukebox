...
// ==========================================
var navidromeServer = "http://141.144.192.216:4533";
var user = "test"; 
var pass = "123456"; 
var salt = ""; // Laat dit helemaal leeg tussen de aanhalingstekens
...
function laadMijnCollectie() {
    ...
    Papa.parse(CSV_BESTAND, {
        ...
        error: function() {
            updateStatus("Fout: Kon CSV bestand niet vinden.");
        }
    });
}
    ...
    elpees.forEach((elpee) => {
        ...
        kaartje.innerHTML = `
            ...
            <div class="album-info">
                <h2 style="font-size:1rem; margin:10px 0 5px 0; color:white;">${elpee.Artist || 'Onbekend'}</h2>
                <p style="color:#888; margin:0; font-size:0.9rem;">${elpee.Title || 'Onbekend'}</p>
            </div>`;
...
async function startFotoWachtrij() {
    if (laadIndex >= alleElpees.length) { updateStatus("Alle gegevens geladen."); return; }
    ...
    laadIndex++;
    setTimeout(startFotoWachtrij, 2000);
}
...
async function openMuziekPopup(elpee) {
    ...
    details.innerHTML = `<h2 style="color:white; padding:40px;">Server scannen naar tracks...</h2>`;
    ...
    if (!data) {
        try {
            ...
            data = await res.json();
            discogsCache[id] = data;
        } catch(e) { details.innerHTML = "Discogs data niet bereikbaar."; return; }
    }
    ...
    for (const track of data.tracklist || []) {
        ...
        try {
            // Nieuwe Navidrome authenticatie met Token en Salt
            const searchUrl = `${navidromeServer}/rest/search3.view?u=${user}&t=${pass}&s=${salt}&v=1.16.1&c=jukebox&query=${encodeURIComponent(schoneTitel)}&f=json`;
            const response = await fetch(searchUrl);
            const sData = await response.json();
            const songs = sData["subsonic-response"]?.searchResult3?.song;
            const gevonden = songs?.find(s => s.title.toLowerCase().includes(schoneTitel.toLowerCase()));

            if (gevonden) {
                mp3Url = `${navidromeServer}/rest/stream?u=${user}&t=${pass}&s=${salt}&v=1.16.1&c=jukebox&id=${gevonden.id}`;
                bestaat = true;
            }
        ...
        tracklistHtml += `
            <div style="margin-bottom:15px; border-bottom:1px solid #333; padding:10px; text-align:left;">
                <div style="color:white;">
                    <b style="color:#ecc94b;">${track.position}</b> ${track.title} ${bestaat ? '✅' : '❌'}
                </div>
                ${bestaat ? `<audio controls style="width:100%; height:30px; margin-top:5px;"><source src="${mp3Url}" type="audio/mpeg"></audio>` : '<p style="color:#ff4444; font-size:0.8rem;">Niet gevonden op server</p>'}
            </div>`;
    }
...
window.onclick = (e) => { if (e.target == document.getElementById('album-modal')) { document.getElementById('album-modal').style.display = "none"; } };
